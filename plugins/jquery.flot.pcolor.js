    ...
    function init(plot) {
        ...
        function processOptions(plot,options){
            ...
            if(options.series.pcolor.active){
				...
				plot.pcolor.colormap = octx.getImageData(0,0,16385,1);
				
				// creates off-screen canvas and context for plot rendering
				plot.pcolor.ocnv = document.createElement("canvas");
				plot.pcolor.octx = plot.pcolor.ocnv.getContext("2d");
				// fills scale bar
				if (plot.pcolor.scalebar.show) {
					sctx.fillStyle = cgrd;
					sctx.fillRect(0,0,plot.pcolor.scalebar.grdcnv.width,plot.pcolor.scalebar.grdcnv.height);
					sctx.rect(0,0,plot.pcolor.scalebar.grdcnv.width, plot.pcolor.scalebar.grdcnv.height);
					sctx.stroke();
					// parses label format
					plot.pcolor.scalebar.labelformat = options.series.pcolor.scalebar.labelformat.charAt(options.series.pcolor.scalebar.labelformat.length-1);
					plot.pcolor.scalebar.labelprecision = parseInt(options.series.pcolor.scalebar.labelformat);
				}
            }
        }
				...
				if (plot.pcolor.scalebar.show) {
					...
					if (serie.pcolor.scalebar.labels>1) {
						...
						for (var t=0; t<serie.pcolor.scalebar.labels; t++) {
							var tv = t*dt + c_min;
							switch(plot.pcolor.scalebar.labelformat) {
							case "e":
								labels[t] = tv.toExponential(plot.pcolor.scalebar.labelprecision);
								break;
							case "f":
								labels[t] = tv.toFixed(plot.pcolor.scalebar.labelprecision);
								break;
							case "p":
								labels[t] = tv.toPrecision(plot.pcolor.scalebar.labelprecision);
								break;
							case "c":
								labels[t] = plot.pcolor.scalebar.labelformatter(tv,plot.pcolor.scalebar.labelprecision);
							default:
								labels[t] = "";
							}
					...
					switch (plot.pcolor.scalebar.position) {
					case 0: // inside top left
						if (plot.pcolor.scalebar.orientation) {
							var cpx = offset.left+plot.pcolor.scalebar.grdcnv.height;
							var cpy = offset.top+plot.pcolor.scalebar.grdcnv.height;
							var ocpx = 0;
							var ocpy = tmh+4;
							var otx = 0;
							var oty = tmh+2;
						} else {
							var cpx = offset.left+plot.pcolor.scalebar.grdcnv.width;
							var cpy = offset.top+plot.pcolor.scalebar.grdcnv.width;
							var ocpx = plot.pcolor.scalebar.grdcnv.width;
							var ocpy = 0;
							var otx = 2;
							var oty = tmh;
						}
					...
					case 1: // inside top right
						if (plot.pcolor.scalebar.orientation) {
							var cpx = pw-offset.right-plot.pcolor.scalebar.grdcnv.width-plot.pcolor.scalebar.grdcnv.height;
							var cpy = offset.top+plot.pcolor.scalebar.grdcnv.height;
							var ocpx = 0;
							var ocpy = tmh+4;
							var otx = 0;
							...
							var cpx = pw-offset.right-2*plot.pcolor.scalebar.grdcnv.width;
							var cpy = offset.top+plot.pcolor.scalebar.grdcnv.width;
							var ocpx = -tmw-4;
							var ocpy = 0;
							var otx = 2;
					...
					case 2: // inside bottom right
						if (plot.pcolor.scalebar.orientation) {
							var cpx = pw-offset.right-plot.pcolor.scalebar.grdcnv.width-plot.pcolor.scalebar.grdcnv.height;
							var cpy = ph-offset.bottom-2*plot.pcolor.scalebar.grdcnv.height;
							var ocpx = 0;
							var ocpy = -tmh-4;
							var otx = 0;
							var oty = tmh;
							...
							var cpx = pw-offset.right-2*plot.pcolor.scalebar.grdcnv.width;
							var cpy = ph-offset.bottom-plot.pcolor.scalebar.grdcnv.height-plot.pcolor.scalebar.grdcnv.width;
							var ocpx = -tmw-4;
							var ocpy = 0;
							var otx = 2;
							var oty = tmh;
					...
					if (serie.pcolor.scalebar.labels>1) {
						...
						if (plot.pcolor.scalebar.orientation) {
							...
							for (t=0; t<serie.pcolor.scalebar.labels; t++) {
								...
								if (plot.pcolor.scalebar.textalign==3) { // spread labels
									if (t==0) {
									...
									ctx.fillText(labels[t],labx+cdw*t+corrw*plot.pcolor.scalebar.textalign, laby);
								}
							}
						} else {
							// for vertical orientation
							ctx.fillRect(cpx+ocpx, cpy+ocpy, tmw+4, plot.pcolor.scalebar.grdcnv.height);
							ctx.fillStyle="#000000";
							var cdh = (plot.pcolor.scalebar.grdcnv.height-oty-2)/(serie.pcolor.scalebar.labels-1);
							for (t=0; t<serie.pcolor.scalebar.labels; t++) {
								var corrw = (tmw-lwidths[t])/2;
								ctx.fillText(labels[t],cpx+ocpx+otx+corrw*plot.pcolor.scalebar.textalign, cpy+ocpy+plot.pcolor.scalebar.grdcnv.height-2-cdh*t);
							}
						}
					}
					ctx.drawImage(plot.pcolor.scalebar.grdcnv,cpx,cpy);
					ctx.restore();
				}
